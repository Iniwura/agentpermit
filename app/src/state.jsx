...
const scope = (suffix) => `agentpermit.${suffix}.v5.${api.CONTRACT_ADDRESS.toLowerCase()}`;
...
const TX_KEYS = {
  ...
  revokePermit: `agentpermit.revokePermit.v5.${api.CONTRACT_ADDRESS.toLowerCase()}`,
};
...
const scopedCanExecute = async (action) => {
  if (action?.decision !== "PERMITTED" || !action.target || !action.payload_hash) return false;
  return api.canExecute(action.id, action.target, action.payload_hash);
};
...
export function PermitProvider({children}) {
  ...
  const reviewsRef = useRef(reviews), transactionsRef = useRef(transactions), disconnected = useRef(false), poll = useRef({timer:0,busy:false,backoff:8000}), revokePoll = useRef({timer:0,busy:false,backoff:8000});
  useEffect(()=>{ reviewsRef.current=reviews; localStorage.setItem(REVIEW_KEY,JSON.stringify(reviews)); },[reviews]);
  useEffect(()=>{ transactionsRef.current=transactions; Object.entries(TX_KEYS).forEach(([kind,key])=>transactions[kind]&&localStorage.setItem(key,JSON.stringify(transactions[kind]))); },[transactions]);
  ...
  const updateTransaction = useCallback((kind, patch) => setTransactions(current => ({...current, [kind]: {...current[kind], ...patch}})), []);
  const syncReview = useCallback(async(id)=>{ const saved=reviewsRef.current[id]; if(!saved?.transactionHash) return saved; try { const tx=await api.getTransaction(saved.transactionHash); let lifecycleStage=stageOf(statusOf(tx),tx), snapshot=saved.snapshot, allowed=saved.canExecute===true; if(lifecycleStage==="FINALIZING"){ try { snapshot=await api.readAction(id); allowed=await scopedCanExecute(snapshot); if(snapshot.status!=="PENDING_REVIEW"){lifecycleStage="FINALIZED";refresh().catch(()=>{});} } catch {lifecycleStage="STATUS_UNAVAILABLE";} } const next={...saved,lifecycleStage,lastKnownStatus:statusOf(tx),snapshot,canExecute:allowed,error:""}; updateReview(id,next); return next; } catch(e){ const next={...saved,lifecycleStage:terminal.has(saved.lifecycleStage)?saved.lifecycleStage:"STATUS_UNAVAILABLE",error:api.errorText(e)}; updateReview(id,next); return next;} },[refresh,updateReview]);
  const syncRevoke = useCallback(async()=>{ const saved=transactionsRef.current.revokePermit; if(!saved?.transactionHash) return saved; const id=Number(saved.key ?? saved.payload?.id); try { const tx=await api.getTransaction(saved.transactionHash); let lifecycleStage=stageOf(statusOf(tx),tx), snapshot=saved.snapshot, allowed=saved.canExecute===true; if(lifecycleStage==="FINALIZING"){ try { snapshot=await api.readAction(id); allowed=await scopedCanExecute(snapshot); if(snapshot.revoked===true){lifecycleStage="FINALIZED";refresh().catch(()=>{});}else{lifecycleStage="FINALIZING";} } catch { lifecycleStage="STATUS_UNAVAILABLE"; } } const next={...saved,lifecycleStage,lastKnownStatus:statusOf(tx),snapshot,canExecute:allowed,error:""}; updateTransaction("revokePermit",next); return next; } catch(e){ const next={...saved,lifecycleStage:terminal.has(saved.lifecycleStage)?saved.lifecycleStage:"STATUS_UNAVAILABLE",error:api.errorText(e)}; updateTransaction("revokePermit",next); return next; } },[refresh,updateTransaction]);
  ...
  useEffect(()=>{const manager=revokePoll.current; const sync=async()=>{if(manager.busy)return;const saved=transactionsRef.current.revokePermit;if(!saved?.transactionHash||!activeStages.has(saved.lifecycleStage))return;manager.busy=true;await syncRevoke();manager.busy=false;const current=transactionsRef.current.revokePermit;const unavailable=current?.lifecycleStage==="STATUS_UNAVAILABLE";manager.backoff=unavailable?Math.min(manager.backoff*1.5,30000):8000;manager.timer=setTimeout(sync,unavailable?manager.backoff:4000);};sync();const wake=()=>{clearTimeout(manager.timer);sync();};const visible=()=>document.visibilityState==="visible"&&wake();window.addEventListener("focus",wake);document.addEventListener("visibilitychange",visible);return()=>{clearTimeout(manager.timer);window.removeEventListener("focus",wake);document.removeEventListener("visibilitychange",visible);};},[transactions,syncRevoke]);
  const value=useMemo(()=>({account,agents,actions,error,loading,reviews,transactions,configured:api.configured,connect,disconnect,refresh,setError,updateReview,syncReview,beginReview,createAgent:(p)=>sign("agentCreation","create",()=>api.createAgent(p),p),proposeAction:(p)=>sign("actionSubmission","propose",()=>api.proposeAction(p),p),updateMandate:(id,text)=>sign("mandateUpdate",String(id),()=>api.updateMandate(id,text),{id,text}),revokePermit:(id)=>sign("revokePermit",String(id),()=>api.revokePermit(id),{id})}),[account,agents,actions,error,loading,reviews,transactions,connect,disconnect,refresh,updateReview,syncReview,beginReview,sign]);
