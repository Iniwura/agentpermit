...
export const CONTRACT_ADDRESS = (
  ...
  || "0xb1DbF4AA85B585652bD2d453CAb2c1426fFB70E8"
).trim();
...
export function normalizeAction(raw, id) { const a = record(raw); return { id: number(id), agent_id: number(a.agent_id), title: String(a.title || ""), description: String(a.description || ""), action_type: String(a.action_type || ""), recipient: String(a.recipient || ""), amount: String(a.amount || ""), asset: String(a.asset || ""), purpose: String(a.purpose || ""), evidence: String(a.evidence || ""), evidence_urls: array(a.evidence_urls).map(String), mandate_version: number(a.mandate_version), proposer: String(a.proposer || ""), status: String(a.status || "PENDING_REVIEW"), decision: String(a.decision || ""), evidence_state: String(a.evidence_state || ""), reasoning: String(a.reasoning || ""), relevant_policy_ids: array(a.relevant_policy_ids).map(String), relevant_policies: String(a.relevant_policies || ""), evidence_summary: String(a.evidence_summary || "") }; }
...
export async function readAll() { if (!configured) return { agents: [], actions: [] }; const [ac, xc] = await Promise.all([read("get_agent_count"), read("get_action_count")]); const agents = await Promise.all(Array.from({length:number(ac)},(_,i)=>readAgent(i+1))); const actions = await Promise.all(Array.from({length:number(xc)},async(_,i)=>{ const action=await readAction(i+1); return {...action,can_execute:action.decision==="PERMITTED"?await canExecute(i+1):false}; })); return { agents, actions: actions.reverse() }; }
...
export const proposeAction = (p) => write("propose_action", [Number(p.agent_id),p.title,p.description,p.action_type,p.recipient,p.amount,p.asset,p.purpose,p.evidence,p.evidence_urls]);
export const reviewAction = (id) => write("review_action", [Number(id)]);
export const canExecute = (id) => read("can_execute", [Number(id)]).then(Boolean);
